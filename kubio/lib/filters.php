...
add_filter(
	...
	function ($parts = []) {
		return array_merge((array) $parts, [
			'core/template-part',
			'kubio/header',
			'kubio/footer',
			'kubio/sidebar',
...
function kubio_blocks_update_template_parts_theme($parsed_blocks, $theme)
{
	...
	Utils::walkBlocks($parsed_blocks, function (&$block) use (
		$theme,
		$parts_block_names
	) {
...
function kubio_register_anchor_attribute($metaData)
{
	...
	if ($supportsAnchor) {
		...
		if (!$hasAnchorAttribute) {
			...
			LodashBasic::set($metaData, ['attributes', 'anchor'], $anchorData);
		}
	}
	...
	function () {
		...
		if (is_page() && !is_front_page() && $is_kubio_theme) {
			...
			if (
				 ...
				 * Page.
				 */
				locate_block_template('full-width', 'page', ['full-width.php']);
				locate_block_template('kubio-full-width', 'page', [
	...
	function () {
		...
		if (is_front_page() && is_home()) {
			...
			if (isset($_GET['_wp-find-template']) && $callFromKubio) {
				locate_block_template('home', 'page', ['home.php']);
				locate_block_template('index', 'page', ['index.php']);
			}
		}
	},
	5
